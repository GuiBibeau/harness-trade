...
describe("worker runtime research synthesis route", () => {
  test("requires admin auth", async () => {
    const { env, sqlite } = createOpsEnv();
    try {
      const response = await worker.fetch(
        new Request(
          "http://localhost/api/admin/ops/runtime/research/synthesis",
          {
            method: "POST",
            headers: {
              "content-type": "application/json",
            },
            body: JSON.stringify({ brief: briefFixture }),
          },
        ),
        env,
        createExecutionContextStub(),
  ...
  test("builds and returns a synthesis artifact through the admin route", async () => {
    const { env, sqlite } = createOpsEnv();
    try {
      const response = await worker.fetch(
        new Request(
      ...
      expect(await response.json()).toMatchObject({
        ...
        synthesis: {
          ...
          implementationPlan: {
            issueTitle: "[Candidate] Trend following",
          },
        },
        markdown: expect.stringContaining("## Candidate Issue"),
      });
    } finally {
      sqlite.close();
    }
  ...
  test("fails closed when the brief payload is invalid", async () => {
    const { env, sqlite } = createOpsEnv();
    try {
      const response = await worker.fetch(
        new Request(
      ...
      expect(await response.json()).toMatchObject({
        ok: false,
        error: "invalid-runtime-research-synthesis-generatedAt",
      });
    } finally {
      sqlite.close();
    }
